...
export const CLIENT_ROUTER_SCRIPT = `(()=>{let D=document,L=location,C={},S=p=>p.length>1&&p.endsWith('/')?p.slice(0,-1):p,N=(u,a)=>{a=new URL(u,L);return a.origin==L.origin?S(a.pathname)+a.search+a.hash:0},F=u=>(u=u.split('#')[0],C[u]||(C[u]=fetch(u).then(r=>{if(!r.ok)throw 0;return r.text()}).catch(()=>delete C[u]))),Y=(q,a)=>{for(a of q.querySelectorAll('link'))D.head.querySelector('[href="'+a.getAttribute('href')+'"]')||D.head.append(a)},X=(r,a)=>r.querySelectorAll('script').forEach(o=>{let s=D.createElement('script');for(a of o.attributes)s.setAttribute(a.name,a.value);s.text=o.text;o.replaceWith(s)}),P=u=>{let n=N(u);n&&!C[n.split('#')[0]]&&F(n).then(t=>Y(new DOMParser().parseFromString(t,'text/html'))).catch(()=>{})},R=()=>{if(L.pathname.length>1&&L.pathname.endsWith('/'))history.replaceState(0,'',S(L.pathname)+L.search+L.hash)},T=async(u,k=1)=>{let n=N(u);if(!n)return;try{let t=await F(n),q=new DOMParser().parseFromString(t,'text/html');D.title=q.title||D.title;D.head.append(...q.querySelectorAll('style'));Y(q);let a=D.querySelector('main'),b=q.querySelector('main');a&&b?a.innerHTML=b.innerHTML:D.body.innerHTML=q.body.innerHTML;X(D.body);R();k&&history.pushState(0,'',n);D.getElementById(u.split('#')[1])?.scrollIntoView()||scroll(0,0);if(D.querySelector('deshi-island'))await import('/_deshi/islands.js');dispatchEvent(new CustomEvent('deshi:navigated',{detail:n}))}catch{L.href=n||u}};D.onclick=e=>{if(e.defaultPrevented||e.button||e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return;let a=e.target.closest('a');if(!a||!a.href||a.target&&a.target!='_self'||a.download)return;let n=N(a.href);if(!n||n==S(L.pathname)+L.search+L.hash)return;e.preventDefault();T(n)};D.onpointerover=e=>{let a=e.target.closest('a');a?.href&&P(a.href)};R();onpopstate=()=>T(S(L.pathname)+L.search+L.hash,0)})()`;

...
Code fences in `.md` / `.mdx` files are highlighted at **build time** with [Shiki](https://shiki.style). Configure the theme and extra languages:

```ts
markdown: {
  shikiConfig: {
    theme: 'github-dark',
    langs: ['ts', 'rust', 'zig'], // extras on top of the defaults
    enabled: true,                // set false to disable
  },
}
```

<Callout type="tip" title="Build-time only">
  Highlighting runs during the build; only the resulting
  `<span style="color:…">` markup ships to the browser. Common web/markdown
  languages are preloaded, and unlabelled or unsupported fences fall back to a

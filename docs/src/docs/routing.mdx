...
| `src/about/page.deshi` (or `src/about.mdx`) | `/about` | Static Segment |
...
└── docs/
    ...
    ├── page.mdx                # Wrapped by: root layout -> docs layout
    └── routing.mdx             # Wrapped by: root layout -> docs layout
```
...
Place a `not-found.deshi`, `not-found.mdx`, or `not-found.md` file at the root of `src/`. During static build, this file automatically compiles to `dist/404.html`, compatible with Cloudflare Pages, Vercel, Netlify, and GitHub Pages.
...
<Callout type="tip" title="Matching is structural">
  Deshijs matches routes by walking segments (static / dynamic / catch-all) rather
  than compiling a regular expression per route. The same matcher runs in the dev
  server, the build, and the optional client router — so dev and production agree.
</Callout>
